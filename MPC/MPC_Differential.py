...
"""def update_state(state, v, omega):
    ...
    state.x = rosbot.robot.odom_pose.position.x
    state.y = rosbot.robot.odom_pose.position.y
    #state.yaw = np.deg2rad(rosbot.robot.local_deg)
    # state.yaw = rosbot.robot.odom_theta
...
def update_state(state, v, omega):
    ...
    state.x = rosbot.robot.pose.position.x
    state.y = rosbot.robot.pose.position.y
    #state.yaw += rosbot.robot.theta - state.yaw
    state.yaw = rosbot.robot.theta 
    return state
...
if __name__ == '__main__':
    ...
    dl = 0.1 # course tick
    ax = [rosbot.robot.pose.position.x, rosbot.robot.pose.position.x+3]
    ay = [rosbot.robot.pose.position.y, rosbot.robot.pose.position.y] 
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(ax, ay, ds=dl)
    sp = calc_speed_profile(cx, cy, cyaw, TARGET_SPEED)
    
    state = State(x=cx[0], y=cy[0], yaw=cyaw[0]) # cyaw[0]
...
while MAX_TIME >= time:
        ...
        rosbot.teszt(speed_x=vi, speed_z=omegai)
        ...
        if target_ind % 1 == 0 and show_animation == True:
            ...
            plt.axis("equal")
            plt.grid(True)
            plt.title("speed[m/sec]:" + str(round(vi, 2))  + "," + str(round(state.x, 2)))
            plt.pause(0.0005)
